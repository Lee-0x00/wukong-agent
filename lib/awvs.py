...
from common.check import awvs_target_check                 #check format

class Work(object):
    def __init__(self, task_id = "", task_target = "", cookie = "" , back_fn = None):
            ...
            for node in ReportItem_list:
                ...
                try:
                    if color in self.filter['awvs_white_list'] and name not in self.filter['bug_black_list']:
                        ...
                        temp['bug_detail'] = "Vulnerability link : " + affect + "  Detail : " + request + response
    ...
    def run(self):
        ...
        if status == 1 :
            ...
            awvs_report = self.awvs_report_task(self.task_id,awvs_id)  
            ...
            if awvs_report_status == 1 :
                ...
                for line in awvs_report_data : 
                    ...
                    save_result("awvs",task_result)
                    self.result.append(task_result)

                #finish
                final_result = { "status" : 1 , "task_id": self.task_id , "model": "awvs" }
                save_result("awvs",final_result)
            ...
            final_result = { "status" : 2 , "data" : awvs_error , "task_id": self.task_id , "model": "awvs" }
            save_result("awvs",final_result)
            self.result.append(final_result)
...
if __name__ == '__main__' :
    ...
    if host != "" :
        try:
            t = Work(task_id = taskid , task_target = host , cookie = cookie)
            t.run()
